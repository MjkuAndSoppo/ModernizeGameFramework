package com.modernizegameframework.looting.network.C2S;

import com.modernizegameframework.looting.client.core.ISuperStack;
import com.modernizegameframework.looting.mixin.ItemEntityAccessor;
...
public class PacketBatchPickup {
     ...
     */
    public static void handle(PacketBatchPickup msg, Supplier<NetworkEvent.Context> ctx) {
        ...
        ctx.get().enqueueWork(() -> {
            ...
            int remainingQuota = msg.limitToMaxStack ? 64 : Integer.MAX_VALUE;
            ...
            for (int id : msg.entityIds) {
                ...
                if (!(target instanceof ItemEntity item) || !item.isAlive() || player.distanceToSqr(item) >= 100.0 ||
                        (msg.isAuto && ((ItemEntityAccessor) item).getPickupDelay() > 0)) continue;
